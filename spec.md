...
**Copyright** &copy; 2026 AFPS contributors. Licensed under [CC-BY-4.0](./LICENSE).
...
AFPS uses a `snake_case` field vocabulary across all package types. The `mcp-server` package type is AFPS-native at the root and adopts the MCP Bundle ([MCPB]) field vocabulary for its server, tools, and user-configuration block; the `integration` package type wraps a capability source (a local `mcp-server`, a remote MCP endpoint, or an HTTP API) with authentication and credential-delivery metadata.
...
The `_meta` object is a record of namespaced keys. Each key is an OPTIONAL reverse-DNS prefix (a dotted label sequence followed by `/`) plus a name; for example `dev.afps/policy` or `com.example/cost-center`. The value at a namespaced key MUST be a JSON object.
...
- Vendors use their own reverse-DNS prefix for implementation-specific data (for example `com.example/`). A vendor MUST use a reverse-DNS prefix derived from a domain it controls.
- Producers MUST NOT use an `mcp` or `modelcontextprotocol` prefix; those are reserved by MCP.

> **Editorial note (namespace control).** The `afps.dev` domain corresponding to the `dev.afps/` prefix is controlled by the AFPS maintainers, so `dev.afps/` is unambiguously the spec-aligned namespace.
