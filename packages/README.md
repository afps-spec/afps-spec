...
`afps-schema@0.1.0`) triggers the shared
...
tool conventions (e.g. an implementation's `log` / `add_memory` /
`set_state` / `output` / `report` shapes) are implementation-defined and
belong in that implementation's own package namespace, not here.
