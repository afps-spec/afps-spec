...
describe("JSON Schema purity — generated schemas", () => {
  const agentSchemaPath = join(import.meta.dir, "../v0/agent.schema.json");
...
describe("cross-field MUST rules injected into JSON Schema (lockstep with Zod superRefine)", () => {
  const integ = JSON.parse(
    readFileSync(join(import.meta.dir, "../v0/integration.schema.json"), "utf-8"),
  );
  const mcp = JSON.parse(
    readFileSync(join(import.meta.dir, "../v0/mcp-server.schema.json"), "utf-8"),
  );
